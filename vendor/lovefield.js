...
(function () {
    ...
    goog.DEPENDENCIES_ENABLED && (goog.dependencies_ = {
        ...
        if (goog.isDef(goog.global.CLOSURE_BASE_PATH)) {
            ...
            if (goog.inHtmlDocument_()) {
                for (var doc = goog.global.document, scripts = doc.getElementsByTagName("SCRIPT"), i = scripts.length - 1; 0 <= i; --i) {
                    var script = scripts[i], src = script.src, qmark = src.lastIndexOf("?"),
        ...
        var path = goog.getPathFromDeps_(name), loadFlags = path && goog.dependencies_.loadFlags[path] || {},
            languageLevel = loadFlags.lang || "es3";
        ...
        for (i = 0; i < scripts.length; i++) {
            if (path$jscomp$0 = scripts[i]) {
                var loadFlags = deps.loadFlags[path$jscomp$0] || {}, languageLevel = loadFlags.lang || "es3",
                    needsTranspile = goog.needsTranspile_(languageLevel);
    ...
    goog.workaroundSafari10EvalBug = function (moduleDef) {
        var srcUrlRE = /\/\/# sourceURL.*\n/g, srcMapUrlRE = /\/\/# sourceMappingURL.*\n/g,
            srcUrlLine = goog.getLastREMatch_(moduleDef, srcUrlRE),
            srcMapUrlLine = goog.getLastREMatch_(moduleDef, srcMapUrlRE);
    ...
    goog.transpile_ = function (code$jscomp$0, path$jscomp$0) {
        ...
        if (!transpile) {
            var suffix = " requires transpilation but no transpiler was found.",
                suffix = suffix + ' Please add "//javascript/closure:transpiler" as a data dependency to ensure it is included.',
    ...
    goog.string.toTitleCase = function (str, opt_delimiters) {
        var delimiters = goog.isString(opt_delimiters) ? goog.string.regExpEscape(opt_delimiters) : "\\s",
            ...
            regexp = new RegExp("(^" + delimiters + ")([a-z])", "g");
...
*/
    ...
    lf.backstore.BaseTx.prototype.mergeTableChanges_ = function () {
        ...
        diff.forEach(function (tableDiff, tableName) {
            var tableSchema = this.journal_.getScope().get(tableName),
                table = this.getTable(tableSchema.getName(), tableSchema.deserializeRow.bind(tableSchema), lf.backstore.TableType.DATA),
                toDeleteRowIds = lf.structs.map.values(tableDiff.deleted_).map(function (row) {
                    return row.id();
                });
        ...
        if (goog.isNull(this.stats_)) {
            if (this.success_) {
                if (this.txType == lf.TransactionType.READ_ONLY) {
                    ...
                    var diff = this.journal_.tableDiffs_, insertedRows = 0, deletedRows = 0, updatedRows = 0,
                        tablesChanged = 0;
    ...
    lf.backstore.BundledObjectStore.prototype.getPagesByRowIds_ = function (rowIds) {
        var results = lf.structs.map.create(), resolver = goog.Promise.withResolver(),
            pageIds = lf.backstore.Page.toPageIds(rowIds), promises = pageIds.map(function (id) {
                return new goog.Promise(function (resolve, reject) {
                    var request;
                    try {
    ...
    lf.backstore.BundledObjectStore.getDataTablePage_ = function (global, tableName, pageId) {
        var cache = global.getService(lf.service.CACHE), range = lf.backstore.Page.getPageRange(pageId),
            rows = cache.getRange(tableName, range[0], range[1]), page = new lf.backstore.Page(pageId);
        page.setRows(rows);
        return page;
    };
    ...
    lf.cache.InMemoryUpdater.prototype.updateTableIndexForRow_ = function (index, modification) {
        var keyNow = goog.isNull(modification[1]) ? void 0 : modification[1].keyOfIndex(index.getName()),
            keyThen = goog.isNull(modification[0]) ? void 0 : modification[0].keyOfIndex(index.getName());
    ...
    lf.cache.ConstraintChecker.prototype.findExistingRowIdInIndex_ = function (indexSchema, row) {
        var indexName = indexSchema.getNormalizedName(), indexKey = row.keyOfIndex(indexName),
            index = this.indexStore_.get(indexName), rowIds = index.get(indexKey);
    ...
    lf.cache.ConstraintChecker.prototype.findParentIndex_ = function (foreignKeySpec) {
        var parentTable = this.schema_.table(foreignKeySpec.parentTable),
            parentColumn = parentTable[foreignKeySpec.parentColumn], parentIndexSchema = parentColumn.getIndex();
    ...
    lf.cache.ConstraintChecker.prototype.loopThroughReferringRows_ = function (foreignKeySpecs, modifications, callbackFn) {
        foreignKeySpecs.forEach(function (foreignKeySpec) {
            var childIndex = this.indexStore_.get(foreignKeySpec.name),
                parentIndex = this.getParentIndex_(foreignKeySpec);
            modifications.forEach(function (modification) {
    ...
    lf.cache.ConstraintChecker.prototype.detectCascadeDeletion = function (table$jscomp$0, rows) {
        var result = {tableOrder: [], rowIdsPerTable: new lf.structs.MapSet},
            lastRowIdsToDelete = new lf.structs.MapSet;
        ...
        do {
            ...
            lastRowIdsToDelete.keys().forEach(function (tableName) {
                var table = this.schema_.table(tableName), rowIds = lastRowIdsToDelete.get(tableName),
                    modifications = rowIds.map(function (rowId) {
    ...
    lf.cache.Journal.prototype.insertOrReplace = function (table, rows) {
        ...
        this.constraintChecker_.checkNotNullable(table, rows);
        for (var i = 0; i < rows.length; i++) {
            var rowNow = rows[i], rowBefore = null,
                existingRowId = this.constraintChecker_.findExistingRowIdInPkIndex(table, rowNow);
        ...
        if (!goog.isNull(foreignKeySpecs)) {
            ...
            cascadedUpdates.keys().forEach(function (rowId) {
                ...
                updates.forEach(function (update) {
                    var table = this.schema_.table(update.fkSpec.childTable), rowBefore = this.cache_.get(rowId),
                        rowAfter = table.deserializeRow(rowBefore.serialize());
    ...
    lf.cache.Journal.prototype.removeByCascade_ = function (table$jscomp$0, rows$jscomp$0) {
        var foreignKeySpecs = this.schema_.info().getReferencingForeignKeys(table$jscomp$0.getName(), lf.ConstraintAction.CASCADE);
        if (!goog.isNull(foreignKeySpecs)) {
            var cascadeDeletion = this.constraintChecker_.detectCascadeDeletion(table$jscomp$0, rows$jscomp$0),
                cascadeRowIds = cascadeDeletion.rowIdsPerTable;
    ...
    lf.index.SingleKeyRange.prototype.contains = function (key) {
        var left = lf.index.SingleKeyRange.isUnbound(this.from) || key > this.from || key == this.from && !this.excludeLower,
            right = lf.index.SingleKeyRange.isUnbound(this.to) || key < this.to || key == this.to && !this.excludeUpper;
        return left && right;
    };
    ...
    lf.index.SingleKeyRange.compareKey_ = function (l, r, isLeftHandSide, opt_excludeL, opt_excludeR) {
        var Favor = lf.index.Favor, excludeL = opt_excludeL || !1, excludeR = opt_excludeR || !1,
            flip = function (favor) {
    ...
    lf.index.SingleKeyRange.and = function (r1, r2) {
        ...
        var r = lf.index.SingleKeyRange.all(), favor = lf.index.SingleKeyRange.compareKey_(r1.from, r2.from, !0),
            left = favor == lf.index.Favor.TIE ? r1.excludeLower ? r1 : r2 : favor != lf.index.Favor.RHS ? r1 : r2;
        r.from = left.from;
    ...
    lf.proc.QueryTask.prototype.exec = function () {
        var journal = this.txType_ == lf.TransactionType.READ_ONLY ? void 0 : new lf.cache.Journal(this.global, this.combinedScope_),
            results = [], remainingPlans = this.plans_.slice(), queries = this.queries, sequentiallyExec = function () {
            ...
            if ("@rev" != child.key()) {
                ...
                if (!removedIds.has(rowId)) {
                    var row = child.val(), diff = diffs.get(row.T), table = this.tables_.get(diff.getName()),
                        tableSchema = this.schema_.table(diff.getName()),
                        nowRow = tableSchema.deserializeRow({id: rowId, value: row.P});
    ...
    lf.backstore.Firebase.prototype.reloadTable = function (tableName) {
        var resolver = goog.Promise.withResolver(), tid = this.getTableId(tableName),
            tableSchema = this.schema_.table(tableName);
    ...
    lf.backstore.IndexedDB.prototype.onUpgradeNeeded_ = function (onUpgrade, ev) {
        var db = ev.target.result, tx = ev.target.transaction,
            rawDb = new lf.backstore.IndexedDBRawBackStore(ev.oldVersion, db, tx, this.bundledMode_);
    ...
    lf.backstore.WebSqlTable.prototype.get = function (ids) {
        var where = 0 == ids.length ? "" : "WHERE id IN (" + ids.join(",") + ")",
            sql = "SELECT id, value FROM " + this.name_ + " " + where, deserializeFn = this.deserializeFn_,
    ...
    lf.backstore.WebSqlTable.prototype.remove = function (ids) {
        var where = 0 == ids.length ? "" : "WHERE id IN (" + ids.join(",") + ")",
            sql = "DELETE FROM " + this.name_ + " " + where;
        this.tx_.queue(sql, []);
        return goog.Promise.resolve();
    };
    ...
    lf.backstore.WebSql.prototype.checkVersion_ = function (onUpgrade) {
        var resolver = goog.Promise.withResolver(),
            tx = new lf.backstore.WebSqlTx(this.db_, lf.TransactionType.READ_WRITE, this.getEmptyJournal_());
    ...
    lf.backstore.WebSql.prototype.preUpgrade_ = function () {
        var tables = this.schema_.tables(),
            tx = new lf.backstore.WebSqlTx(this.db_, lf.TransactionType.READ_WRITE, this.getEmptyJournal_()),
            tx2 = new lf.backstore.WebSqlTx(this.db_, lf.TransactionType.READ_WRITE, this.getEmptyJournal_());
    ...
    lf.cache.DefaultCache.prototype.getRange = function (tableName, fromId, toId) {
        var data = [], min = Math.min(fromId, toId), max = Math.max(fromId, toId),
            tableSet = this.tableRows_.get(tableName);
    ...
    lf.index.BTree.prototype.getAll_ = function (maxCount, reverse, limit, skip) {
        var offset = reverse ? this.stats_.totalRows - maxCount - skip : skip, results = Array(maxCount),
            params = {offset: offset, count: maxCount, startIndex: 0};
    ...
    lf.index.BTree.prototype.getRange = function (opt_keyRanges, opt_reverseOrder, opt_limit, opt_skip) {
        ...
        var reverse = opt_reverseOrder || !1,
            limit = goog.isDefAndNotNull(opt_limit) ? Math.min(opt_limit, this.stats_.totalRows) : this.stats_.totalRows,
            skip = opt_skip || 0, maxCount = Math.min(Math.max(this.stats_.totalRows - skip, 0), limit);
        if (0 == maxCount) {
            return lf.index.BTree.EMPTY;
        }
        ...
        var sortedKeyRanges = this.comparator_.sortKeyRanges(opt_keyRanges),
            results = Array(reverse ? this.stats_.totalRows : maxCount),
            params = {count: 0, limit: results.length, reverse: reverse, skip: skip},
            useFilter = 1 < this.comparator().keyDimensions();
    ...
    lf.index.BTree.deserialize = function (comparator, name, uniqueKeyOnly, rows) {
        var tree = new lf.index.BTree(name, comparator, uniqueKeyOnly),
            newRoot = lf.index.BTreeNode_.deserialize(rows, tree);
        tree.root_ = newRoot;
        return tree;
    };
    ...
    lf.index.BTreeNode_.dumpLevel_ = function (node$jscomp$0) {
        var key = node$jscomp$0.id_ + "[" + node$jscomp$0.keys_.join("|") + "]",
            childrenIds = node$jscomp$0.children_.map(function (n) {
                return n.id_;
            }), children = childrenIds.join("|"), values = node$jscomp$0.values_.join("/"),
        ...
        if (node$jscomp$0.next_) {
            var next = lf.index.BTreeNode_.dumpLevel_(node$jscomp$0.next_), key = key + "  " + next[0],
    ...
    lf.index.BTreeNode_.createLeaves_ = function (tree, data) {
        for (var remaining = data.length, dataIndex = 0, curNode = lf.index.BTreeNode_.create(tree), node = curNode; 0 < remaining;) {
            var nodeLen = lf.index.BTreeNode_.calcNodeLen_(remaining),
                target = data.slice(dataIndex, dataIndex + nodeLen);
    ...
    lf.index.BTreeNode_.createInternals_ = function (node) {
        ...
        if (data.length <= lf.index.BTreeNode_.MAX_KEY_LEN_ + 1) {
            ...
            for (root.height_ = node.height_ + 2; 0 < remaining;) {
                var nodeLen = lf.index.BTreeNode_.calcNodeLen_(remaining),
                    target = data.slice(dataIndex, dataIndex + nodeLen),
                    newNode = lf.index.BTreeNode_.createParent_(target);
    ...
    lf.index.BTreeNode_.prototype.insert = function (key, value, opt_replace) {
        ...
        var pos = this.tree_.eq(this.keys_[pos], key) ? pos + 1 : pos,
            node = this.children_[pos].insert(key, value, opt_replace);
    ...
    lf.index.BTreeNode_.prototype.splitLeaf_ = function () {
        var half = lf.index.BTreeNode_.MIN_KEY_LEN_, right = lf.index.BTreeNode_.create(this.tree_),
            root = lf.index.BTreeNode_.create(this.tree_);
        root.height_ = 1;
    ...
    lf.index.BTreeNode_.prototype.splitInternal_ = function () {
        var half = lf.index.BTreeNode_.MIN_KEY_LEN_, root = lf.index.BTreeNode_.create(this.tree_),
            right = lf.index.BTreeNode_.create(this.tree_);
        root.parent_ = this.parent_;
    ...
    lf.index.BTreeNode_.prototype.getRange = function (keyRange, params, results) {
        var c = this.tree_.comparator(), left = 0, right = this.keys_.length - 1, compare = function (coverage) {
            ...
            }, keys = this.keys_, favorLeft = compare(c.compareRange(keys[left], keyRange)),
            favorRight = compare(c.compareRange(keys[right], keyRange));
    ...
    lf.index.SimpleComparator.prototype.compareRange = function (key, naturalRange) {
        var range = this.normalizeKeyRange_(naturalRange),
            results = [lf.index.SingleKeyRange.isUnbound(range.from), lf.index.SingleKeyRange.isUnbound(range.to)];
        if (!results[0]) {
    ...
    lf.index.RowId.prototype.getRange = function (opt_keyRanges, opt_reverseOrder, opt_limit, opt_skip) {
        var keyRanges = opt_keyRanges || [lf.index.SingleKeyRange.all()],
            values = lf.structs.set.values(this.rows_).filter(function (value) {
    ...
    lf.cache.Prefetcher.prototype.init = function (schema) {
        var tables = schema.tables(), execSequentially = function () {
            ...
            var table = tables.shift(),
                whenTableFetched = table.persistentIndex() ? this.fetchTableWithPersistentIndices_(table) : this.fetchTable_(table);
    ...
    lf.cache.Prefetcher.prototype.fetchTable_ = function (table) {
        var tx = this.backStore_.createTx(lf.TransactionType.READ_ONLY, [table]),
            store = tx.getTable(table.getName(), table.deserializeRow.bind(table), lf.backstore.TableType.DATA),
            promise = store.get([]).then(function (results) {
    ...
    lf.cache.Prefetcher.prototype.fetchTableWithPersistentIndices_ = function (tableSchema) {
        var tx = this.backStore_.createTx(lf.TransactionType.READ_ONLY, [tableSchema]),
            store = tx.getTable(tableSchema.getName(), tableSchema.deserializeRow, lf.backstore.TableType.DATA),
            whenTableContentsFetched = store.get([]).then(function (results) {
    ...
    lf.cache.Prefetcher.prototype.reconstructPersistentIndex_ = function (indexSchema, tx) {
        var indexTable = tx.getTable(indexSchema.getNormalizedName(), lf.Row.deserialize, lf.backstore.TableType.INDEX),
            comparator = lf.index.ComparatorFactory.create(indexSchema);
            ...
            if (0 < serializedRows.length) {
                if (indexSchema.hasNullableColumn()) {
                    var deserializeFn = lf.index.BTree.deserialize.bind(void 0, comparator, indexSchema.getNormalizedName(), indexSchema.isUnique),
                        nullableIndex = lf.index.NullableIndex.deserialize(deserializeFn, serializedRows);
    ...
    lf.index.MemoryIndexStore.createIndex_ = function (indexSchema) {
        var comparator = lf.index.ComparatorFactory.create(indexSchema),
            index = new lf.index.BTree(indexSchema.getNormalizedName(), comparator, indexSchema.isUnique);
    ...
    lf.pred.JoinPredicate.prototype.createCombinedEntryForUnmatched_ = function (entry, leftRelationTables) {
        ...
        var nullEntry = new lf.proc.RelationEntry(new lf.Row(lf.Row.DUMMY_ID, this.nullPayload_), !1),
            combinedEntry = lf.proc.RelationEntry.combineEntries(entry, leftRelationTables, nullEntry, [this.rightColumn.getTable().getEffectiveName()]);
            ...
            for (var i = 0; i < leftEntriesLength; i++) {
                ...
                if (!goog.isNull(leftValue)) {
                    for (var rightLimit = Math.min(currentBlock + 1 << blockNumBits, rightEntriesLength), j = currentBlock << blockNumBits; j < rightLimit; j++) {
                        ...
                        if (predicateResult) {
                            var matchFound = !0,
                                combinedEntry = lf.proc.RelationEntry.combineEntries(leftRelation.entries[i], leftRelationTables, rightRelation.entries[j], rightRelationTables);
    ...
    lf.pred.JoinPredicate.prototype.evalRelationsHashJoin = function (leftRelation, rightRelation, isOuterJoin) {
        ...
        leftRelation = leftRightRelations[0];
        rightRelation = leftRightRelations[1];
        var minRelation = leftRelation, maxRelation = rightRelation, minColumn = this.leftColumn,
            maxColumn = this.rightColumn;
    ...
    lf.pred.JoinPredicate.prototype.evalRelationsIndexNestedLoopJoin = function (leftRelation, rightRelation, indexJoinInfo, cache) {
        ...
        var indexedTable = indexJoinInfo.indexedColumn.getTable(), outerRelation = leftRelation,
            innerRelation = rightRelation;
        -1 != leftRelation.getTables().indexOf(indexedTable.getEffectiveName()) && (outerRelation = rightRelation, innerRelation = leftRelation);
        var combinedEntries = [], innerRelationTables = innerRelation.getTables(),
            outerRelationTables = outerRelation.getTables(), pushCombinedEntry = function (outerEntry, row) {
                var innerEntry = new lf.proc.RelationEntry(row, 1 < innerRelationTables.length),
                    combinedEntry = lf.proc.RelationEntry.combineEntries(outerEntry, outerRelationTables, innerEntry, innerRelationTables);
                combinedEntries.push(combinedEntry);
            };
        outerRelation.entries.forEach(function (entry) {
            var keyOfIndex = this.keyOfIndexFn_(entry.getField(indexJoinInfo.nonIndexedColumn)),
                matchingRowIds = indexJoinInfo.index.get(keyOfIndex);
    ...
    lf.proc.AggregationStep.Calculator_.prototype.calculate = function () {
        this.columns_.forEach(function (column) {
            for (var reverseColumnChain = column.getColumnChain().reverse(), i = 1; i < reverseColumnChain.length; i++) {
                var currentColumn = reverseColumnChain[i], leafColumn = currentColumn.getColumnChain().slice(-1)[0],
                    inputRelation = this.getInputRelationFor_(currentColumn);
    ...
    lf.proc.AndPredicatePass.prototype.traverse_ = function (rootNode) {
        if (rootNode instanceof lf.proc.SelectNode) {
            ...
            var predicates = this.breakAndPredicate_(rootNode.predicate),
                newNodes = this.createSelectNodeChain_(predicates);
    ...
    lf.query.DeleteContext.prototype.expandTableScope_ = function (tableName, scopeSoFar) {
        var cascadeChildTables = this.schema.info().getChildTables(tableName, lf.ConstraintAction.CASCADE),
            childTables = this.schema.info().getChildTables(tableName);
    ...
    lf.query.insertToSql_ = function (query, stripValueInfo) {
        var prefix = query.allowReplace ? "INSERT OR REPLACE" : "INSERT", columns = query.into.getColumns(),
            prefix = prefix + (" INTO " + query.into.getName() + "("), prefix = prefix + columns.map(function (col) {
                return col.getName();
    ...
    lf.query.valuePredicateToSql_ = function (pred, stripValueInfo) {
        var column = pred.column.getNormalizedName(), op = lf.query.evaluatorToSql_(pred.evaluatorType),
            value = lf.query.valueToSql_(pred.value, pred.evaluatorType, pred.column.getType(), stripValueInfo);
    ...
    lf.query.getFromListForOuterJoin_ = function (query, stripValueInfo) {
        for (var retrievedNodes = lf.tree.find(query.where, function (node) {
            ...
            var fromName = lf.query.getTableNameToSql_(query.from[i]),
                ...
                fromList = fromList + (" ON (" + predicateString[predicateString.length - i] + ")");
        }
        var node$jscomp$0 = query.where,
            leftChild = 0 < node$jscomp$0.getChildCount() ? node$jscomp$0.getChildAt(0) : node$jscomp$0;
    ...
    lf.query.SelectBuilder.prototype.checkDistinctColumn_ = function () {
        var distinctColumns = this.query.columns.filter(function (column) {
            ...
            isValidCombination = 0 == distinctColumns.length || 1 == distinctColumns.length && 1 == this.query.columns.length;
    ...
    lf.proc.UpdateLogicalPlanGenerator.prototype.generateInternal = function () {
        var updateNode = new lf.proc.UpdateNode(this.query.table),
            selectNode = goog.isDefAndNotNull(this.query.where) ? new lf.proc.SelectNode(this.query.where.copy()) : null,
            tableAccessNode = new lf.proc.TableAccessNode(this.query.table);
    ...
    lf.proc.DeleteLogicalPlanGenerator.prototype.generateInternal = function () {
        var deleteNode = new lf.proc.DeleteNode(this.query.from),
            selectNode = goog.isDefAndNotNull(this.query.where) ? new lf.proc.SelectNode(this.query.where.copy()) : null,
            tableAccessNode = new lf.proc.TableAccessNode(this.query.from);
    ...
    lf.proc.ImplicitJoinsPass.prototype.traverse_ = function (rootNode, queryContext) {
        if (rootNode instanceof lf.proc.SelectNode && rootNode.predicate instanceof lf.pred.JoinPredicate) {
            ...
            if (child$jscomp$0 instanceof lf.proc.CrossProductNode) {
                var isOuterJoin = goog.isDefAndNotNull(queryContext.outerJoinPredicates) && queryContext.outerJoinPredicates.has(predicateId),
                    joinNode = new lf.proc.JoinNode(rootNode.predicate, isOuterJoin);
        ...
        var processChildren = function (node) {
            ...
            if (!this.alreadyPushedDown_.has(node)) {
                if (this.isCandidateNode_(node)) {
                    var selectNode = node, selectNodeTables = selectNode.predicate.getTables(),
                        shouldPushDownFn = function (child) {
    ...
    lf.proc.PushDownSelectionsPass.prototype.shouldSwapWithChild_ = function (queryContext, node) {
        ...
        var nodeIsJoin = node.predicate instanceof lf.pred.JoinPredicate,
            childIsOuterJoin = queryContext.outerJoinPredicates.has(child.predicate.getId());
    ...
    lf.proc.SelectLogicalPlanGenerator.prototype.generateInternal = function () {
        ...
        var rootNode = this.connectNodes_(),
            planRewriter = new lf.proc.LogicalPlanRewriter(rootNode, this.query, this.rewritePasses_);
        return planRewriter.generate();
    };
    ...
    lf.proc.GetRowCountStep.prototype.execInternal = function () {
        var rowIdIndex = this.indexStore_.get(this.table.getRowIdIndexName()),
            relation = new lf.proc.Relation([], [this.table.getName()]);
    ...
    lf.proc.IndexJoinPass.prototype.processJoinStep_ = function (joinStep) {
        if (joinStep.predicate.evaluatorType == lf.eval.Type.EQ && !joinStep.isOuterJoin) {
            var getCandidate = function (executionStep) {
                ...
                }, leftCandidate = getCandidate(joinStep.getChildAt(0)),
                rightCandidate = getCandidate(joinStep.getChildAt(1));
    ...
    lf.proc.BoundKeyRangeCalculator.prototype.calculateKeyRangeMap_ = function (queryContext) {
        ...
        this.predicateMap_.keys().forEach(function (columnName) {
            var predicateIds = this.predicateMap_.get(columnName),
                predicates = predicateIds.map(function (predicateId) {
    ...
    lf.proc.IndexRangeCandidate.prototype.calculateCost = function (queryContext) {
        var combinations = this.getKeyRangeCalculator().getKeyRangeCombinations(queryContext),
            indexData = this.indexStore_.get(this.indexSchema.getNormalizedName());
    ...
    lf.proc.IndexRangeScanStep.prototype.execInternal = function (relations, opt_journal, opt_context) {
        var context = opt_context, keyRanges = this.keyRangeCalculator.getKeyRangeCombinations(context),
            index = this.indexStore_.get(this.index.getNormalizedName()), rowIds;
        ...
        tableAccessFullSteps.forEach(function (tableAccessFullStep) {
            ...
            if (0 != selectStepsCandidates.length) {
                var costEstimator = new lf.proc.IndexCostEstimator(this.global_, tableAccessFullStep.table),
                    indexRangeCandidate = costEstimator.chooseIndexFor(queryContext, selectStepsCandidates.map(function (c) {
    ...
    lf.proc.IndexRangeScanPass.prototype.replaceWithIndexRangeScanStep_ = function (indexRangeCandidate, predicateToSelectStepMap, tableAccessFullStep) {
        var predicateIds = indexRangeCandidate.getPredicateIds(),
            selectSteps = predicateIds.map(function (predicateId) {
                return predicateToSelectStepMap.get(predicateId);
            });
        selectSteps.forEach(lf.tree.removeNode);
        var indexRangeScanStep = new lf.proc.IndexRangeScanStep(this.global_, indexRangeCandidate.indexSchema, indexRangeCandidate.getKeyRangeCalculator(), !1),
            tableAccessByRowIdStep = new lf.proc.TableAccessByRowIdStep(this.global_, tableAccessFullStep.table);
    ...
    lf.proc.InsertStep.assignAutoIncrementPks_ = function (table, values, indexStore) {
        var pkIndexSchema = table.constraint_.primaryKey_,
            autoIncrement = goog.isNull(pkIndexSchema) ? !1 : pkIndexSchema.columns[0].autoIncrement;
        if (autoIncrement) {
            var pkColumnName = pkIndexSchema.columns[0].schema.getName(),
                index = indexStore.get(pkIndexSchema.getNormalizedName()), max = index.stats().maxKeyEncountered,
                maxKey = goog.isNull(max) ? 0 : max;
    ...
    lf.proc.OrderByStep.prototype.execInternal = function (relations) {
        if (1 == relations.length) {
            var distinctColumn = this.findDistinctColumn_(relations[0]),
                relationToSort = goog.isNull(distinctColumn) ? relations[0] : relations[0].getAggregationResult(distinctColumn);
    ...
    lf.proc.RelationTransformer.prototype.handleAggregatedColumns_ = function () {
        if (1 == this.columns_.length && this.columns_[0].aggregatorType == lf.fn.Type.DISTINCT) {
            var distinctRelation = this.relation_.getAggregationResult(this.columns_[0]),
                newEntries = distinctRelation.entries.map(function (entry) {
    ...
    lf.proc.RelationTransformer.prototype.handleNonAggregatedColumns_ = function () {
        var transformedEntries = Array(this.relation_.entries.length),
            isPrefixApplied = this.relation_.isPrefixApplied();
    ...
    lf.proc.RelationTransformer.transformMany = function (relations, columns) {
        var entries = relations.map(function (relation) {
            var relationTransformer = new lf.proc.RelationTransformer(relation, columns),
                singleEntryRelation = relationTransformer.getTransformed();
    ...
    lf.proc.MultiColumnOrPass.prototype.findIndexRangeCandidates_ = function (selectStep, queryContext) {
        ...
        var tableSchema = lf.structs.set.values(tables)[0],
            indexCostEstimator = new lf.proc.IndexCostEstimator(this.global_, tableSchema), indexRangeCandidates = null,
            allIndexed = predicate.getChildren().every(function (childPredicate) {
    ...
    lf.proc.MultiColumnOrPass.prototype.replaceWithIndexRangeScan_ = function (selectStep, tableAccessFullStep, indexRangeCandidates) {
        var tableAccessByRowIdStep = new lf.proc.TableAccessByRowIdStep(this.global_, tableAccessFullStep.table),
            multiIndexRangeScanStep = new lf.proc.MultiIndexRangeScanStep;
    ...
    lf.proc.OrderByIndexPass.prototype.applyTableAccessFullOptimization_ = function (orderByStep) {
        var rootNode = orderByStep,
            tableAccessFullStep = lf.proc.OrderByIndexPass.findTableAccessFullStep_(orderByStep.getChildAt(0));
    ...
    lf.proc.OrderByIndexPass.prototype.applyIndexRangeScanStepOptimization_ = function (orderByStep) {
        var rootNode = orderByStep,
            indexRangeScanStep = lf.proc.OrderByIndexPass.findIndexRangeScanStep_(orderByStep.getChildAt(0));
        if (!goog.isNull(indexRangeScanStep)) {
    ...
    lf.proc.OrderByIndexPass.getIndexCandidateForIndexSchema_ = function (indexSchema, orderBy) {
        ...
        return isNaturalOrReverse[0] || isNaturalOrReverse[1] ? {
    ...
    lf.proc.OrderByIndexPass.checkOrder_ = function (orderBy, indexSchema) {
        var ordersLeftBitmask = orderBy.reduce(function (soFar, columnOrderBy) {
            ...
            }, 0), xorBitmask = ordersLeftBitmask ^ ordersRightBitmask, isNatural = 0 == xorBitmask,
            isReverse = xorBitmask == Math.pow(2, Math.max(orderBy.length, indexSchema.columns.length)) - 1;
    ...
    lf.proc.Runner.prototype.requestTwoPhaseLock_ = function (task, lockType1, lockType2) {
        var acquiredLock = !1,
            acquiredFirstLock = this.lockManager_.requestLock(task.getId(), task.getScope(), lockType1);
    ...
    lf.debug.inspectTable_ = function (dbName, tableName, opt_limit, opt_skip) {
        ...
        if (goog.isDefAndNotNull(global)) {
            ...
            if (goog.isDefAndNotNull(table)) {
                var indexStore = global.getService(lf.service.INDEX_STORE), cache = global.getService(lf.service.CACHE),
                    rowIds = indexStore.get(table.getRowIdIndexName()).getRange(void 0, !1, opt_limit, opt_skip);
    ...
    lf.ObserverRegistry.prototype.updateResultsForQuery = function (query, results) {
        var queryId = this.getQueryId_(goog.isDefAndNotNull(query.clonedFrom) ? query.clonedFrom : query),
            entry = this.entries_.get(queryId) || null;
    ...
    lf.base.init = function (global, opt_options) {
        var schema = global.getService(lf.service.SCHEMA), options = opt_options || {},
    ...
    lf.proc.ExportTask.prototype.execSync = function () {
        var indexStore = this.global_.getService(lf.service.INDEX_STORE),
            cache = this.global_.getService(lf.service.CACHE), tables = {};
        this.schema_.tables().forEach(function (table) {
            var rowIds = indexStore.get(table.getRowIdIndexName()).getRange(),
                payloads = cache.getMany(rowIds).map(function (row) {
                    return row.payload_;
                });
    ...
    lf.proc.ImportTask.prototype.import_ = function () {
        var journal = new lf.cache.Journal(this.global_, this.scope_),
            tx = this.backStore_.createTx(this.getType(), lf.structs.set.values(this.scope_), journal), tableName;
        for (tableName in this.data_.tables) {
            var tableSchema = this.schema_.table(tableName), payloads = this.data_.tables[tableName],
                rows = payloads.map(function (value) {
    ...
    lf.schema.Info.prototype.getReferencingForeignKeys = function (tableName, opt_constraintAction) {
        ...
        var cascadeConstraints = this.cascadeReferringFk_.get(tableName),
            restrictConstraints = this.restrictReferringFk_.get(tableName);
    ...
    lf.schema.TableBuilder.prototype.generateTableClass_ = function () {
        var that = this, tableClass = function () {
            ...
            var pk = goog.isNull(that.pkName_) ? null : new lf.schema.Index(that.name_, that.pkName_, !0, generateIndexedColumns.call(this, that.pkName_)),
                notNullable = columns.filter(function (col) {
    ...
    lf.schema.TableBuilder.prototype.generateRowClass_ = function (columns$jscomp$0, indices) {
        ...
        var getSingleKeyFn = function (column) {
            var colType = this.columns_.get(column.getName()),
                keyOfIndexFn = this.evalRegistry_.getKeyOfIndexEvaluator(colType);
    ...
    lf.schema.Builder.prototype.checkForeignKeyValidity_ = function (builder) {
        ...
        fkSpecArray.forEach(function (specs) {
            ...
            var table = this.tableBuilders_.get(parentTableName), parentSchema = table.getSchema(),
                parentColName = specs.parentColumn;
